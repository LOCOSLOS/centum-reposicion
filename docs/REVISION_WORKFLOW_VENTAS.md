...
3. `ventas_items` utiliza `ON CONFLICT (id_venta, id_articulo)`. Actualmente no existen repeticiones, pero una venta futura podría contener dos líneas legítimas del mismo artículo.
...
Antes de cargar 12 o 24 meses de histórico, preparar una segunda versión del workflow y una migración de Supabase que:
